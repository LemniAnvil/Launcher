...
public enum MojangEndpoint: Equatable {
    ...
    case authenticateMojang(username: String, password: String, clientToken: String)
    ...
    var path: String {
        ...
        case .getPlayerProfile(let uuid):
            return "/users/profiles/minecraft/\(uuid.uuidString.replacingOccurrences(of: "-", with: ""))"
    ...
    var method: String {
        switch self {
        case .getPlayerUUID, .getPlayerProfile, .getNameHistory, .getSessionProfile, .getMinecraftProfile, .getBlockedServers, .checkServerStatus:
            return "GET"
        default:
            return "POST"
        }
