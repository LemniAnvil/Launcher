...
public class PlayerService {
  ...
  public func getPlayerProfile(uuid: UUID) async throws -> PlayerProfile {
    let response: SessionProfile = try await client.request(
  ...
  private func parseSessionProfile(_ response: SessionProfile) throws
    -> PlayerProfile
  {
    var skins: [Skin] = []
    var capes: [Cape] = []
        ...
        if let skinInfo = texturesData.textures.skin {
          let skin = Skin(
            ...
            metadata: SkinMetadata(
              model: skinInfo.metadata?.model == "slim" ? .alex : .steve
            ),
            state: .active
          )
          skins.append(skin)
  ...
  private func decodeTexturesProperty(_ base64String: String) throws
    -> TexturesProperty
  {
